...
def f(x,t):
    return k*(np.cos(np.pi*x)-np.exp(-1*np.pi**2*t))
...
def objfun(x,t):
    return k*(1-np.exp(-1*np.pi**2*t))*(np.cos(np.pi*x)-1)/np.pi**2
...
#Convergence order of forward difference O(tau+h) a*tau/h^2<=0.5
#tau
h=1/10
taus=[1/200,1/220,1/250,1/260]
max_error=[]
for tau in taus:
    ...
    result=Parabolic_Equation_forward2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
...
#Convergence order of backward difference O(tau+h)
#tau
h=1/1000
...
for tau in taus:
    ...
    for i in range(M+1):
        for j in range(N+1):
            true[i,j]=objfun(x_range[0]+j*h,t_range[0]+i*tau)
    result=Parabolic_Equation_backward2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
    error=np.abs(true-result)
    max_error.append(error.max())
taus2=['{:.4f}'.format(i) for i in taus]
max_error2=['{:.4f}'.format(i) for i in max_error]  
...
for tau in taus:
    ...
    for i in range(M+1):
        for j in range(N+1):
            true[i,j]=objfun(x_range[0]+j*h,t_range[0]+i*tau)
    result=Parabolic_Equation_CN2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
    error=np.abs(true-result)
    max_error.append(error.max())
taus2=['{:.4f}'.format(i) for i in taus]
max_error2=['{:.4f}'.format(i) for i in max_error]  
...
h=1/20
tau=1/1000
result1=Parabolic_Equation_forward2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
result2=Parabolic_Equation_backward2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
result3=Parabolic_Equation_CN2(h,tau,a,f,x_range,t_range,u0t,u1t,ux0,2,1)
...
Z_true=k*(1-np.exp(-1*np.pi*np.pi*Y4))*(np.cos(np.pi*X4)-1)/np.pi**2
