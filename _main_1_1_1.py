...
h2=np.pi/80
test_h=[np.pi/7,np.pi/9,np.pi/11,np.pi/14]
max_diff=[]
x_range=[0,math.pi]
y_range=[0,math.pi]
for h in test_h:
...
h1=np.pi/90
test_h=[np.pi/10,np.pi/12,np.pi/15,np.pi/18]
max_diff=[]
x_range=[0,math.pi]
y_range=[0,math.pi]
for h in test_h:
...
Z_true=np.sin(X2)*np.sin(Y2)

fig=plt.figure()
fig.suptitle('Visualization of problem 1(a=1, b=0, h1=pi/30, h2=pi/30)')
